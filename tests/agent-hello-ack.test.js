...
async function run() {
    try {
        ...
        const socket = io(BASE_URL, {
            auth: { token: SECRET, agent_id: 'hello-tester', role: 'worker', specs: {} },
        ...
        const res = await new Promise((resolve, reject) => {
            socket.timeout(1000).emit('agent_hello', { capabilities: { offline_outbox: true } }, (err, payload) => {
                if (err) return reject(err);
                resolve(payload);
            });
        });

        assert(res && res.ok === true, 'Expected ok:true from agent_hello ack');
        socket.disconnect();
        console.log('✓ agent_hello ack works');
    } finally {
        await stopServer();
    }
}
...
run().catch((err) => {
    console.error(`✗ agent_hello ack failed: ${err && err.message ? err.message : String(err)}`);
    process.exit(1);
});
